# 🔄 Complete Request Flow: React → Backend → Response
...
┌─────────────────────────────────────────────────────────────────────────┐
│                          REACT FRONTEND                                  │
│  (User fills form: Name, Policy No, Death Certificate, Claim Amount)   │
└────────────────────────────────┬────────────────────────────────────────┘
                                 │
                                 │ HTTP POST Request
                                 │ /api/claim/submit
...
│                      SPRING BOOT BACKEND                                 │
│  ┌───────────────────────────────────────────────────────────────────┐  │
│  │  1️⃣  ClaimController.submitClaim()                                │  │
│  │      - Receives HTTP request                                      │  │
│  │      - Extracts: name, policyNumber, claimAmount, files          │  │
│  └──────────────────────────┬────────────────────────────────────────┘  │
│                             │                                            │
│                             │ Calls                                      │
│                             ▼                                            │
│  ┌───────────────────────────────────────────────────────────────────┐  │
│  │  2️⃣  BlobStorageService.uploadFiles()                            │  │
│  │      ┌──────────────────────────────────────────────────────┐    │  │
│  │      │  For each file:                                       │    │  │
...
│                             ▼                                            │
│  ┌───────────────────────────────────────────────────────────────────┐  │
│  │  3️⃣  PolicyRuleService.checkPolicyExists()                       │  │
│  │      ┌──────────────────────────────────────────────────────┐    │  │
│  │      │  🔍 Check if policy exists in database (FAST)        │    │  │
│  │      │     - Query: policyRepository.findByPolicyNumber()   │    │  │
│  │      │     - If NOT found → REJECT immediately ❌           │    │  │
│  │      │     - If found → Continue to OCR (saves cost!)       │    │  │
│  │      └──────────────────────────────────────────────────────┘    │  │
│  └──────────────────────────┬────────────────────────────────────────┘  │
│                             │                                            │
│                             │ Policy exists, proceed to OCR              │
│                             ▼                                            │
│  ┌───────────────────────────────────────────────────────────────────┐  │
│  │  4️⃣  DocumentAIService.extractText()                             │  │
│  │      ┌──────────────────────────────────────────────────────┐    │  │
│  │      │  For each document URL from Blob Storage:            │    │  │
│  │      │  - Pass Azure Blob URL (NOT image bytes!)            │    │  │
│  │      │  - Send to Azure Document Intelligence               │    │  │
│  │      │  - Document AI downloads from URL directly           │    │  │
│  │      │  - Extract text using OCR (EXPENSIVE)                │    │  │
...
│                             ▼                                            │
│  ┌───────────────────────────────────────────────────────────────────┐  │
│  │  5️⃣  PolicyRuleService.validateClaim()                           │  │
│  │      → Calls AzureOpenAIService (with RAG + LangChain4j)         │  │
│  │      ┌──────────────────────────────────────────────────────┐    │  │
│  │      │  📚 Step 1: RAG - Load Policy Rules                  │    │  │
│  │      │     - Load text files from policy-rules/ folder:     │    │  │
│  │      │       • eligibility.txt                              │    │  │
│  │      │       • death-verification.txt                       │    │  │
│  │      │       • claim-amount.txt                             │    │  │
│  │      │       • fraud-indicators.txt                         │    │  │
│  │      │       • documentation.txt                            │    │  │
│  │      │     - Convert to embeddings using text-embedding    │    │  │
│  │      │     - Store in in-memory vector store               │    │  │
│  │      └──────────────────────────────────────────────────────┘    │  │
│  │      ┌──────────────────────────────────────────────────────┐    │  │
│  │      │  🤖 Step 2: Create AI Agent with Tools               │    │  │
...
│  │      │       • RAG knowledge base (policy rules)            │    │  │
│  │      │     - Agent uses tools to validate                  │    │  │
│  │      │     - GPT-4o analyzes everything                     │    │  │
...
│  │      - Create Claim entity with:                                  │  │
│  │        • claimantName                                             │  │
│  │        • policyNumber                                             │  │
│  │        • claimAmount                                              │  │
│  │        • status (APPROVED/REJECTED)                               │  │
│  │        • aiResponse (reasoning)                                   │  │
│  │        • documentUrls (blob storage URLs)                         │  │
│  │        • submissionDate                                           │  │
...
│                             ▼                                            │
│  ┌───────────────────────────────────────────────────────────────────┐  │
│  │  7️⃣  ClaimController builds response                             │  │
│  │      - Create ClaimResponse DTO with:                             │  │
│  │        • message: "Claim processed successfully"                  │  │
│  │        • claimStatus: "APPROVED"                                  │  │
│  │        • aiResponse: "Approved because..."                        │  │
│  │        • claimId: 123                                             │  │
...
│  - Shows success message: "Claim Approved! ✅"                          │
│  - Displays AI reasoning                                                │
│  - Shows claim ID for tracking                                          │
└─────────────────────────────────────────────────────────────────────────┘
```

---

## 🔄 External Azure Services Used
...
│                                                                   │
│  ┌────────────────────────┐    ┌─────────────────────────┐      │
│  │  Azure Blob Storage    │───▶│  Azure Document AI      │      │
│  │  - Stores uploaded     │    │  - Downloads from URL   │      │
│  │    death certificates  │    │  - OCR text extraction  │      │
│  │  - Returns URLs        │    │  - Reads PDFs, images   │      │
│  │                        │    │  - Returns text         │      │
│  └────────────────────────┘    └─────────────────────────┘      │
│                                                                   │
│  ┌────────────────────────┐    ┌─────────────────────────┐      │
│  │  Azure OpenAI GPT-4o   │    │  Azure SQL Database     │      │
│  │  - AI decision making  │    │  - Stores policies      │      │
│  │  - Uses RAG knowledge  │    │  - Stores claims        │      │
│  │  - Agent with tools    │    │  - Permanent storage    │      │
│  └────────────────────────┘    └─────────────────────────┘      │
│                                                                   │
│  ┌────────────────────────┐                                      │
...
│  │                                                          │    │
│  │  Methods:                                               │    │
│  │  • submitClaim(@RequestParam String claimantName,      │    │
│  │                @RequestParam String policyNumber,       │    │
│  │                @RequestParam Double claimAmount,        │    │
│  │                @RequestParam List<MultipartFile> files) │    │
│  │                                                          │    │
│  │  Returns: ResponseEntity<ClaimResponse>                 │    │
│  └────────────────────────────────────────────────────────┘    │
└────────────────────────┬───────────────────────────────────────┘
                         │
                         │ Injects & Calls
...
│  │                                                          │    │
│  │  Methods:                                               │    │
│  │  • uploadFiles(List<MultipartFile> files)              │    │
│  │    → Returns: List<String> (blob URLs)                 │    │
│  │                                                          │    │
│  │  • generateUniqueFileName(String originalName)         │    │
│  │    → Returns: String (UUID-based filename)             │    │
│  └────────────────────────────────────────────────────────┘    │
│                                                                  │
│  ┌────────────────────────────────────────────────────────┐    │
...
│  │  • String endpoint (from properties)                   │    │
│  │  • String apiKey (from properties)                     │    │
│  │                                                          │    │
│  │  Methods:                                               │    │
│  │  • extractTextFromDocument(String documentUrl)         │    │
│  │    → Returns: String (extracted text)                  │    │
│  │                                                          │    │
│  │  • analyzeDocument(String url)                         │    │
...
│  │                                                          │    │
│  │  Methods:                                               │    │
│  │  • validateClaim(String policyNumber,                  │    │
│  │                  Double claimAmount,                    │    │
│  │                  String extractedText)                  │    │
│  │    → Returns: String (AI decision)                     │    │
│  │                                                          │    │
│  │  • checkPolicyEligibility(Policy policy)               │    │
│  │    → Returns: boolean                                   │    │
│  └────────────────────────────────────────────────────────┘    │
│                                                                  │
│  ┌────────────────────────────────────────────────────────┐    │
...
│  │                                                          │    │
│  │  Methods:                                               │    │
│  │  • @PostConstruct loadPolicyRules()                    │    │
│  │    → Loads RAG knowledge base at startup               │    │
│  │                                                          │    │
│  │  • makeClaimDecision(Policy policy,                    │    │
│  │                      String extractedText,              │    │
│  │                      Double claimAmount)                │    │
│  │    → Creates AI Agent with tools                       │    │
│  │    → Returns: String (AI decision)                     │    │
│  │                                                          │    │
│  │  Inner Class: ClaimAdjusterAgent                       │    │
│  │  • @Tool checkPolicyEligibility(...)                   │    │
│  │  • @Tool validateClaimAmount(...)                      │    │
│  │  • @Tool checkFraudIndicators(...)                     │    │
│  │  • @Tool verifyDocumentation(...)                      │    │
│  │  • @Tool checkTimeLimits(...)                          │    │
│  └────────────────────────────────────────────────────────┘    │
└────────────────────────┬───────────────────────────────────────┘
                         │
                         │ Uses
...
│  │  • String aiResponse (AI reasoning)                    │    │
│  │  • String documentUrls (Blob URLs)                     │    │
...
│  │  Fields:                                                │    │
│  │  • String message                                       │    │
│  │  • String claimStatus (APPROVED/REJECTED)              │    │
│  │  • String aiResponse                                    │    │
│  │  • Long claimId                                         │    │
│  └────────────────────────────────────────────────────────┘    │
└─────────────────────────────────────────────────────────────────┘
...
#### **3️⃣ DocumentAIService.extractTextFromDocument()**
```java
...
#### **4️⃣ PolicyRuleService.validateClaim()**
```java
...
#### **5️⃣ AzureOpenAIService.makeClaimDecision()**
```java
...
public class AzureOpenAIService {
    ...
    public String validateClaimAmount(String policyNumber, Double requestedAmount) {
        ...
        if (requestedAmount <= sumAssured) {
            return String.format("Valid: Requested ₹%.2f is within sum assured ₹%.2f", 
                               requestedAmount, sumAssured);
        }
        return String.format("Invalid: Requested ₹%.2f exceeds sum assured ₹%.2f", 
                           requestedAmount, sumAssured);
    }
...
```
ClaimController
    ├── @Autowired BlobStorageService
    ├── @Autowired DocumentAIService
    ├── @Autowired PolicyRuleService
    └── @Autowired ClaimRepository

BlobStorageService
    └── Uses: Azure BlobServiceClient (External SDK)

DocumentAIService
    └── Uses: Azure DocumentAnalysisClient (External SDK)

PolicyRuleService
    ├── @Autowired PolicyRepository
    └── @Autowired AzureOpenAIService

AzureOpenAIService
    ├── @Autowired PolicyRepository
    ├── Uses: LangChain4j ChatLanguageModel
    ├── Uses: LangChain4j EmbeddingModel
    └── Uses: LangChain4j EmbeddingStore
...
- If policy NOT found → **REJECT immediately** ❌
...
### **Step 4: Upload Files to Cloud** ☁️
...
- ✅ **More efficient**: No need to load file into memory twice
- ✅ **Azure-optimized**: Azure services communicate directly with each other (faster)
- ✅ **No size limits**: Large files don't need to pass through our server
- ✅ **Less bandwidth**: We don't re-upload the same file
...
### **Step 6: AI Makes the Decision** 🤖 (Most Important!)

This is where the magic happens! `AzureOpenAIService` does:

#### **6a. Load Knowledge Base (RAG)** 📚
...
The AI:
1. Calls `checkPolicyEligibility()` → "Policy active ✅"
2. Calls `validateClaimAmount()` → "₹5L < ₹10L sum assured ✅"
3. Calls `checkFraudIndicators()` → "No red flags ✅"
4. Calls `verifyDocumentation()` → "Death cert valid ✅"
5. Makes decision: **APPROVED** ✅

Returns response:
```
APPROVED: All validations passed. Policy is active, claimant is 
registered nominee, death certificate is valid, claim amount 
₹5,00,000 is within sum assured limit of ₹10,00,000.
```
...
User sees this on their screen! 🎊

---

## 🔑 Key Technologies Used at Each Step
...
```
React Request → Controller:          50ms
Upload to Blob Storage:               2-3 seconds
Database Policy Check:                100ms ⚡ (BEFORE OCR!)
...
1. **React** → Sends form data with death certificate
2. **Controller** → Receives and coordinates everything
3. **Blob Storage** → Stores the PDF file (fast, ~2 seconds)
4. **Database** → Checks if policy exists (FAST! ~100ms) ⚡
5. **Document AI** → Reads text from PDF (expensive, only if policy exists)
6. **AI + RAG** → Analyzes everything using policy rules and makes decision
7. **Database** → Saves the claim permanently
8. **React** → Shows result to user

The whole process is **automated** - no human needed to review! The AI acts like a claims adjuster, reading documents, checking policies, and making decisions based on company rules. 🚀

---

**📌 Note**: The AI's decision is based on:
...
Everything happens in **one API call** - user submits, AI processes, user gets result! ⚡
